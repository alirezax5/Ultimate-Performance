...
final class Engine {
        ...
        private function begin_capture() {
                ...
                // Prevent PHP notices/warnings from being injected into the cached HTML.
                // WP_DEBUG_LOG still logs them; we only suppress display during capture.
                if ( ! defined( 'WP_DEBUG' ) || ! WP_DEBUG ) {
                        @ini_set( 'display_errors', 0 ); // phpcs:ignore
                } elseif ( defined( 'WP_DEBUG_DISPLAY' ) && WP_DEBUG_DISPLAY ) {
                        @ini_set( 'display_errors', 0 ); // phpcs:ignore -- prevent notices from polluting cached HTML
                }
